...
static short process_body(sqlite3_context *context,
    ...
    if (fmt && str && flen && slen) {
      ...
      // (especially if they were spaces, removed earlier), but not more
      while (i < thseps) {
...
extern void pg_to_number(sqlite3_context *context,
                         int              argc,
                         sqlite3_value  **argv) {
   ...
   if (ksu_prm_ok(context, argc, argv, "to_number",
     ...
     switch (typ) {
         ...
         default: // BLOB ? Won't do anyhow.
              ksu_err_msg(context, KSU_ERR_N_INV_DATATYPE, 1, "to_number");
              return;
     }
     if ((strcasecmp((char *)sqlite3_value_text(argv[1]), "tm9") == 0)
     ...
     str[slen] = '\0';
     // Get rid of text
     while ((p = strchr(fmt, '"')) != (char *)NULL) {
       *p++ = ' ';
       if ((s = strchr(p, '"')) != (char *)NULL) {
         *s = '\0';
         (void)strncpy(txtstr, p, FORMAT_LEN);
         *s = ' ';
         if ((s = strstr(str, txtstr)) != (char *)NULL) {
            for (i = 0; i < strlen(txtstr); i++) {
               *s++ = ' ';
               *p++ = ' ';
            }
         } else {
     ...
     // Handle sign
     // With PostgreSQL we can find
     //       MI
     //       PR
     //       PL (only + sign)
     //       SG
     //       S
     if ((f = strcasestr(fmt, "MI")) != (char *)NULL) {
       if ((s = strchr(str, '-')) != (char *)NULL) {
          *s = ' ';
          neg = 1;
       }
       *f++ = ' ';
       *f = ' ';
     } else if ((f = strcasestr(fmt, "PR")) != (char *)NULL) {
       if (((s = strchr(str, '<')) != (char *)NULL)
           && ((s2 = strchr(s, '>')) != (char *)NULL)) {
          *s = ' ';
          *s2 = ' ';
          neg = 1;
       }
       *f++ = ' ';
       *f = ' ';
     } else if ((f = strcasestr(fmt, "PL")) != (char *)NULL) {
       if ((s = strchr(str, '+')) != (char *)NULL) {
          *s = ' ';
          neg = 0;
       } else {
       ...
       *f++ = ' ';
       *f = ' ';
     } else if ((f = strcasestr(fmt, "SG")) != (char *)NULL) {
       if (((s = strchr(str, '-')) != (char *)NULL)
           || ((s = strchr(str, '+')) != (char *)NULL)) {
          neg = (*s == '-');
          *s = ' ';
       }
       *f++ = ' ';
       *f = ' ';
     } else if ((f = strcasestr(fmt, "S")) != (char *)NULL) {
       if (((s = strchr(str, '-')) != (char *)NULL)
           || ((s = strchr(str, '+')) != (char *)NULL)) {
          neg = (*s == '-');
          *s = ' ';
     ...
     // Handle ordinal
     //
     if ((f = strcasestr(fmt, "TH")) != (char *)NULL) {
       // We don't check whether case matches ...
       if (((s = strcasestr(str, "ST")) != (char *)NULL)
           || ((s = strcasestr(str, "ND")) != (char *)NULL)
           || ((s = strcasestr(str, "TH")) != (char *)NULL)) {
          *s++ = ' ';
          *s = ' ';
       } else {
          ksu_err_msg(context, KSU_ERR_CONV, "to_number");
