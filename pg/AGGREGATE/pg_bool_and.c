...
extern void pg_bool_and_step(sqlite3_context *context,
                            int              argc,
                            sqlite3_value  **argv) {
  CONTEXT_T *ctx;
  int        typ;
  ...
  if (typ != SQLITE_NULL) {
    if (typ == SQLITE_INTEGER) {
      ...
      if ((val != 0) && (val != 1)) {
        ksu_err_msg(context, KSU_ERR_INV_DATATYPE, "bool_and");
        return;
      }
    } else {
...
extern void pg_bool_and_final(sqlite3_context *context) {
  CONTEXT_T *ctx;
