...
extern void ora_hextoraw(sqlite3_context * context, 
                         int               argc, 
                         sqlite3_value  ** argv) {
           unsigned char *s;
           char          *p;
    ...
    if (len % 2) {
      // Prepend a 0 if the number of characters is odd
      char first[2];
      first[0] = '0';
      first[1] = *s;
    ...
    sqlite3_result_blob(context, (char *)result, rlen, sqlite3_free);
}
